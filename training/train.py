...
LOWER_LIMIT = os.environ.get("LOWER_LIMIT", 13)
UPPER_LIMIT = os.environ.get("UPPER_LIMIT", 15)
...
def run_recommender(train_df, k):
    ...
    recommender = hpfrec.HPF(k=30, random_seed=123,
...
def save_model(s3_client, recommender):
    try:
        status = s3_client.write_pickle_file(os.path.join("maven", DEPLOYMENT_PREFIX, 
                                             MODEL_VERSION, "intermediate-model/hpf_model.pkl"),
                                             recommender)
...
def save_hyperparams(s3_client, content_json):
    try:
        status = s3_client.write_json_file(os.path.join("maven", 
                              DEPLOYMENT_PREFIX,
                              MODEL_VERSION, "intermediate-model/hyperparameters.json"), 
                              content_json)
...
def save_obj(s3_client, trained_recommender, precision, recall, lower_lim, upper_lim):
    logging.info("Trying to save the model.")
    save_model(s3_client, trained_recommender)
    contents = {
        "Minimum length of manifest": lower_lim,
        "Maximum length of manifest": upper_lim,
        "Precision": precision,
        "Recall": recall
    }
...
def train_model():
    ...
    print("User Item DF is: ", user_item_df)
    training_df, testing_df = train_test_split(user_item_df)
    trained_recommender = run_recommender(training_df, 300)
    precision, recall = precision_recall_at_m(5, testing_df, trained_recommender, user_item_df)
    try:
        save_obj(s3_obj, trained_recommender, precision, recall, LOWER_LIMIT, UPPER_LIMIT)  
    except Exception as error:
        logger.error(error)
        raise
