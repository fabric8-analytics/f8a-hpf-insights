...
# -*- coding: utf-8 -*-
"""
Job that will be used to train a model on EMR.
...
def submit_job(input_bootstrap_file, input_src_code_file):
    ...
    # S3 bucket/key, where the input spark job (source code) will be uploaded.
    s3_bucket = config.DEPLOYMENT_PREFIX + '-automated-analytics-spark-jobs'
    s3_key = '{}_{}_insights_training_job.zip'.format(config.DEPLOYMENT_PREFIX,
    ...
    s3_log_key = '{}_{}_spark_emr_log_'.format(config.DEPLOYMENT_PREFIX,
                                               config.COMPONENT_PREFIX,
                                               str_cur_time)
    ...
    response = emr_client.run_job_flow(
        Name="{}_{}_{}".format(config.DEPLOYMENT_PREFIX, config.COMPONENT_PREFIX, str_cur_time),
        LogUri=s3_log_uri,
        ...
        Instances={
            ...
            'InstanceGroups': [
                {
                    'Name': '{}_master_group'.format(config.COMPONENT_PREFIX),
                    'InstanceRole': 'MASTER',
                            ...
                            "Configurations": [
                                {
                                    ...
                                    "Properties": {
                                        ...
                                        "AWS_S3_SECRET_KEY_ID": config.AWS_S3_SECRET_ACCESS_KEY
                                    }
                                }
                            ]
                        }
        ...
        Steps=[
            ...
            },
            {
                ...
                'HadoopJarStep': {
                    'Jar': 'command-runner.jar',
                    'Args': ['/bin/sh', '/home/hadoop/src/scripts/setup_pythonpath.sh']
                }
            },
            {
                ...
                'HadoopJarStep': {
                    'Jar': 'command-runner.jar',
                    'Args': ['python3.6', '/home/hadoop/src/scripts/test_pythonpath.py']
                }
            },
            {
    ...
    if response.get('ResponseMetadata', {}).get('HTTPStatusCode') == 200:
        ...
        output[
            'status_description'] = ("The {} training is in progress. Please check the given "
                                     "training job after some"
                                     " time.".format(config.COMPONENT_PREFIX))
...
if __name__ == "__main__":
    print(submit_job('emr_bootstrap.sh',
                     '{}-{}-tf-training-job.zip'.format(config.DEPLOYMENT_PREFIX,
                                                        config.COMPONENT_PREFIX)))
